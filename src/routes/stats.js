...
router.get('/dashboard', authenticateToken, (req, res) => {
  try {
    ...
    const recentShifts = db.prepare(`
      SELECT id, employee_name, workplace, entry_time, exit_time, duration_minutes, created_at
      FROM shifts 
    ...
    recentShifts.forEach(s => {
      timelineEvents.push({
        type: 'shift',
        id: s.id,
        title: `${s.employee_name} vardiya kaydı`,
        description: `${s.workplace} - Süre: ${(s.duration_minutes / 60).toFixed(1)} saat`,
