...
function renderShiftsTable(shifts) {
  ...
  tbody.innerHTML = shifts.map(shift => {
    ...
    const exitTimeOnly = shift.exit_time ? formatTimeOnly(shift.exit_time) : '<span class="text-amber-400">Devam Ediyor</span>';
    ...
    return `
      <tr class="hover:bg-slate-900/40 transition">
        ...
        <td class="py-3.5 px-4 whitespace-nowrap">
          <span class="px-2.5 py-1 rounded-md text-xs font-semibold ${shift.duration_minutes > 0 ? 'bg-emerald-950/60 text-emerald-300 border border-emerald-500/30' : 'bg-slate-800 text-slate-400'}">
...
async function loadPayments() {
  ...
  try {
    ...
    // Cari ay harcamasını hesapla
    const currentMonth = new Date().toISOString().slice(0, 7);
    const thisMonthSum = currentPaymentsData
      .filter(p => p.payment_date && p.payment_date.startsWith(currentMonth))
      .reduce((sum, p) => sum + (Number(p.amount) || 0), 0);
    document.getElementById('financeSummaryThisMonth').textContent = formatMoney(thisMonthSum);
...
function renderPaymentsTable(payments) {
  ...
  tbody.innerHTML = payments.map(pay => {
    ...
    else if (pay.category === 'Malzeme/Gider') catClass = 'badge-category-malzeme';
...
function exportShiftsToCSV() {
  ...
  const headers = ['ID', 'Tarih', 'Çalışan Adı Soyadı', 'Çalışma Yeri / Şantiye', 'Giriş Saati', 'Çıkış Saati', 'Toplam Süre (Dakika)', 'Hesaplanan Süre', 'Notlar'];
  const rows = currentShiftsData.map(s => [
    ...
    s.exit_time ? s.exit_time.replace('T', ' ') : 'Devam Ediyor',
